...
// ✅ Configuración BROADCAST unilateral (optimizada para archivos grandes)
...
#define ENABLE_INTERLEAVING true  // ✅ Resistencia a pérdidas en ráfaga
#define VEXT 36
#define VEXT_ON LOW
...
void setup() {
  ...
  Serial.println("\n=== TRANSMISOR LoRa BROADCAST v8 (100KB-1MB) ===");
...
void setupWebServer() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    html += "<title>LoRa TX Broadcast v8</title>";
    html += "<style>";
    ...
    html += ".interleave-badge { background: #51cf66; }";
    ...
    html += "<h1>📡 LoRa TX Broadcast v8 (100KB-1MB)</h1>";
    ...
    html += "<strong>ℹ️ OPTIMIZACIONES ACTIVAS:</strong><br>";
    html += "✅ Interleaving para resistir pérdidas en ráfaga<br>";
    html += "✅ FEC (Forward Error Correction) - 1 chunk de paridad por cada 8<br>";
    html += "✅ Manifest repetido para receptores tardíos<br>";
    html += "✅ Soporta archivos hasta 1 MB";
    html += "</div>";
    ...
    html += "<span class='config-badge interleave-badge'>Interleaving: ON</span>";
    html += "</div>";
    html += "<form class='lora-config'>";
    html += "<div class='param-group'><label>📶 Bandwidth (kHz)</label><select name='bw'>";
    html += "<option value='125'" + String(currentBW == 125.0 ? " selected" : "") + ">125 kHz (alcance máx)</option>";
    html += "<option value='250'" + String(currentBW == 250.0 ? " selected" : "") + ">250 kHz (balanceado)</option>";
    html += "<option value='500'" + String(currentBW == 500.0 ? " selected" : "") + ">500 kHz (velocidad máx)</option>";
    html += "</select></div>";
    html += "<div class='param-group'><label>📡 Spreading Factor</label><select name='sf'>";
    html += "<option value='7'" + String(currentSF == 7 ? " selected" : "") + ">SF7 (rápido, corto alcance)</option>";
    html += "<option value='9'" + String(currentSF == 9 ? " selected" : "") + ">SF9 (balanceado)</option>";
    html += "<option value='12'" + String(currentSF == 12 ? " selected" :  "") + ">SF12 (lento, largo alcance)</option>";
    html += "</select></div>";
    html += "<div class='param-group'><label>🔧 Coding Rate</label><select name='cr'>";
    html += "<option value='5'" + String(currentCR == 5 ? " selected" : "") + ">4/5 (menos protección)</option>";
    html += "<option value='7'" + String(currentCR == 7 ? " selected" :  "") + ">4/7 (balanceado)</option>";
    html += "<option value='8'" + String(currentCR == 8 ? " selected" :  "") + ">4/8 (más protección)</option>";
    html += "</select></div>";
    html += "<div class='param-group'><label>🔁 Repeticiones (Carrusel)</label><select name='repeat'>";
    html += "<option value='1'" + String(currentREPEAT == 1 ? " selected" : "") + ">1 vuelta (archivos pequeños)</option>";
    html += "<option value='2'" + String(currentREPEAT == 2 ? " selected" :  "") + ">2 vueltas (recomendado)</option>";
    html += "<option value='3'" + String(currentREPEAT == 3 ? " selected" : "") + ">3 vueltas (ambiente ruidoso)</option>";
    html += "<option value='5'" + String(currentREPEAT == 5 ? " selected" : "") + ">5 vueltas (máxima confiabilidad)</option>";
    html += "</select></div>";
    html += "</form>";
    ...
    if (lastTransmissionTime > 0) {
      ...
      html += "<div class='stat-box'><div class='stat-value'>" + String(currentREPEAT) + "x</div><div class='stat-label'>Vueltas</div></div>";
      html += "</div></div>";
    }
    ...
    html += "<strong>💾 Almacenamiento: </strong> " + String(usedBytes/1024.0, 1) + " KB / " + String(totalBytes/1024.0, 1) + " KB (" + String(usedPercent) + "%)";
    ...
    while (file) {
      if (! file.isDirectory()) {
        ...
        String sizeStr = fileSizeKB < 1.0 ? String(file.size()) + " bytes" : String(fileSizeKB, 2) + " KB";
  ...
  server.on("/upload", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    if (final) {
      uploadFile.close();
      Serial.printf("✅ Subido: %s (%d bytes = %. 2f KB)\n", filename.c_str(), index + len, (index + len)/1024.0);
    }
  });
...
bool sendFile(const char* path) {
  ...
  for (int round = 1; round <= currentREPEAT; round++) {
    ...
    uint8_t fecBlock[FEC_BLOCK_SIZE][CHUNK_SIZE];
    ...
    for (uint16_t i = 0; i < totalChunks; i++) {
      ...
      f.seek((uint32_t)index * CHUNK_SIZE);
