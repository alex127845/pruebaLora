...
struct FileSession {
  uint32_t fileID = 0;
  String fileName = "";
  String tempFileName = "";      // ✅ Archivo temporal durante RX
  ...
  // ✅ Solo flags de recepción (bajo uso de RAM)
...
bool initFileSession(uint32_t fileID, String fileName, uint32_t totalSize, uint16_t totalChunks, uint16_t chunkSize) {
  ...
  // ✅ Solo flags (1 byte por chunk = 4KB para 4096 chunks)
  ...
  // ✅ Crear archivo temporal vacío (pre-alocar espacio)
  ...
  // ✅ Pre-llenar con ceros (reservar espacio)
  uint8_t zeros[256];
  memset(zeros, 0, 256);
  uint32_t written = 0;
...
void setup() {
  ...
  Serial.println("\n=== RECEPTOR LoRa BROADCAST v8 (100KB-1MB) ===");
  ...
  Serial.println("👂 Escuchando broadcast (soporta hasta 1MB)...\n");
}
...
// ✅ Web server optimizado
void setupWebServer() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    String html = "<!DOCTYPE html><html><head>";
    html += "<meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>";
    html += "<title>LoRa RX v8</title>";
    html += "<style>";
    ...
    html += "<h1>🛰️ LoRa RX Broadcast v8 (100KB-1MB)</h1>";
    ...
    html += "<strong>ℹ️ CARACTERÍSTICAS:</strong><br>";
    html += "✅ Escritura incremental (bajo uso de RAM)<br>";
    html += "✅ FEC recovery (recupera chunks perdidos)<br>";
    html += "✅ Soporta interleaving (orden aleatorio)<br>";
    html += "✅ Archivos hasta 1 MB";
    html += "</div>";
    ...
    if (currentSession.active && currentSession.totalChunks > 0) {
      ...
      if (currentSession.chunksReceivedCount > 10) {
        ...
        html += "<p style='text-align:center;margin-top:10px;color:#666;'>⏱️ Tiempo transcurrido: " + String(elapsed) + "s | ETA: ~" + String((int)eta) + "s</p>";
      }
    }
...
// ============================================
// ✅ PROCESAR DATA CHUNK (escritura incremental)
// ============================================
void processDataChunk(uint8_t* data, size_t len) {
  ...
  // ✅ Escribir directamente al archivo temporal en la posición correcta
  File tempFile = LittleFS.open(currentSession.tempFileName, "r+");
  if (!tempFile) {
    Serial.println("❌ Error abriendo temp file");
    return;
  }
  
  uint32_t fileOffset = (uint32_t)chunkIndex * currentSession.chunkSize;
  tempFile.seek(fileOffset);
  tempFile.write(data + idx, dataLen);
  tempFile.close();
  ...
  if (currentPercent >= lastProgressPercent + 5 || currentSession.chunksReceivedCount == currentSession.totalChunks) {
    float progress = (currentSession.chunksReceivedCount * 100.0) / currentSession.totalChunks;
    Serial.printf("📦 Progreso: %u/%u chunks (%.1f%%) | RSSI: %. 1f dBm\n", 
...
void processParityChunk(uint8_t* data, size_t len) {
  ...
  if (!currentSession.parityBlocks[blockIndex].data) return;
...
// ============================================
// ✅ FEC RECOVERY (optimizado para escritura incremental)
// ============================================
void attemptFECRecovery() {
  ...
  File tempFile = LittleFS.open(currentSession.tempFileName, "r+");
  if (!tempFile) {
    Serial.println("❌ Error abriendo temp para FEC");
    return;
  }
  ...
  for (uint16_t block = 0; block < currentSession.numParityBlocks; block++) {
    ...
    if (missingCount == 1 && currentSession.parityBlocks[block].received) {
      ...
      // XOR con chunks existentes del bloque
      uint8_t chunkBuffer[300];
      for (uint16_t i = blockStart; i < blockEnd; i++) {
        if (currentSession.chunkReceived[i]) {
          uint32_t offset = (uint32_t)i * currentSession.chunkSize;
          tempFile.seek(offset);
          size_t readLen = tempFile.read(chunkBuffer, currentSession.chunkSize);
      ...
      tempFile.seek(offset);
      tempFile.write(recovered, maxLen);
...
void finalizeFile() {
  ...
  Serial.printf("⏱️ %. 2f segundos (%.1f min)\n", lastReceptionTime, lastReceptionTime/60.0);
  ...
  Serial.printf("📊 Estadísticas totales: %u paquetes RX | %u errores CRC\n", 
