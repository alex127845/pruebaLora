...
#define ACK_EVERY 5  // ✅ NUEVO: Esperar ACK cada 5 fragmentos
...
void setup() {
  ...
  Serial.println("\n=== TRANSMISOR LoRa + WEB INTERFACE v2 (ACK cada 5 fragmentos) ===");
...
// ✅ FUNCIÓN MEJORADA: ACK cada 5 fragmentos
bool sendFile(const char* path) {
  ...
  for (uint16_t index = 0; index < totalChunks; index++) {
    ...
    while (!success && retries < MAX_RETRIES) {
      ...
      // ✅ CAMBIO PRINCIPAL: Solo esperar ACK cada 5 fragmentos o en el último
      ...
      if (isMultipleOf5 || isLastFragment) {
        ...
        Serial.println("   ⏭️  Sin espera de ACK (próximo en múltiplo de 5)\n");
        success = true;
      }
    }
