...
void setup() {
  ...
  Serial.println("\n=== RECEPTOR LoRa XIAO ESP32-S3 v6 (Validación Robusta) ===");
...
void setupWebServer() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    html += "<title>LoRa Gateway XIAO S3 v6</title>";
    html += "<style>";
    ...
    html += ".btn-apply:hover { background: #45a049; }";
    ...
    html += ".btn-refresh { background: #2196F3; margin-top: 20px; }";
    ...
    html += ".progress-bar { background: linear-gradient(90deg, #4CAF50, #45a049); height: 30px; transition: width 0.3s; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; }";
    html += ".device-info { background: #e3f2fd; padding: 10px; border-radius: 5px; margin-bottom: 15px; font-size: 0.9em; }";
    html += ".warning { background: #fff3cd; border-left: 4px solid #ffc107; padding: 10px; margin: 10px 0; color: #856404; }";
    html += "</style></head><body>";
    html += "<div class='container'>";
    html += "<h1>🛰️ LoRa Gateway XIAO ESP32-S3 v6</h1>";
    
    html += "<div class='device-info'>";
    html += "📟 <strong>XIAO ESP32-S3</strong> | 📡 <strong>SX1262</strong> | 📶 <strong>915 MHz</strong> | 🛡️ <strong>Validación Robusta</strong>";
    html += "</div>";
    ...
    while (file) {
      if (!file.isDirectory()) {
        ...
        html += "<div><div class='file-name'>📄 " + displayName + "</div>";
